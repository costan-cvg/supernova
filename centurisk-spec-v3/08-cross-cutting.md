...
Traditional RBAC rights (View, Add, Edit, Deactivate, Export, Approve, Manage Exposure Types) are valid as a convenience shorthand. Each right maps to a named Cedar policy — "View" is an alias for a policy granting read access scoped to the user's attributes, "Approve" is an alias for a policy granting approval authority scoped by change type and pool. The rights table is an administrative simplification for common access patterns. The Cedar engine always evaluates the underlying policy, not the right name. Hierarchy-based access scoping — assigning a user to a level like Entity, Member, or Site — is one pattern expressible as a Cedar policy prefix filter on the materialized path, not a separate mechanism.
