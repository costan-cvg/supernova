...
test.describe("App Shell", () => {
    test("loads and shows RiskStar nav sidebar", async ({ page, request }) => {
        ...
        await expect(logoText).toHaveText("RiskStar");
    });
    ...
    test("shows logged-in user in header", async ({ page, request }) => {
        ...
        await expect(userInfo).toContainText("RiskStar Admin");
    });
    ...
    test("navigates between sections via sidebar", async ({ page, request }) => {
        ...
        await nav.locator("button", { hasText: "Exposures" }).click();
